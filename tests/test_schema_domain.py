"""
test_schema_domain.py — testy domenowe dla ishkarim_schema.
...
from ishkarim_schema import load_knowledge_index


@pytest.fixture
def repo_root():
    """Zwraca ścieżkę do katalogu głównego repozytorium Ishkarim."""
    root = Path(__file__).resolve().parents[3]  # projects/ishkarim-schema/tests/ → repo root
...
class TestModuleIntegrity:
    ...
    def test_version_format(self):
        assert re.match(r"\d+\.\d+\.\d+", ishkarim_schema.__version__)

    def test_modules_all_strings(self):
        for name in ishkarim_schema.MODULES:
