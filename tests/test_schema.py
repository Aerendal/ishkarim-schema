"""
test_schema.py — testy dla modułu ishkarim_schema.
"""
import pytest
...
class TestImport:
    ...
    def test_import_main(self):
        ...
        assert ishkarim_schema.__version__ == "0.1.0"

    def test_area(self):
        import ishkarim_schema
        assert ishkarim_schema.__area__ == "schema"

    def test_modules_list_nonempty(self):
        import ishkarim_schema
        assert len(ishkarim_schema.MODULES) > 0

    def test_submodule_imports(self):
        from ishkarim_schema import utils
...
class TestKnowledgeIndex:
    ...
    def test_returns_list(self, tmp_path):
        import ishkarim_schema
        result = ishkarim_schema.load_knowledge_index(root=str(tmp_path))
        assert isinstance(result, list)

    def test_record_keys(self, tmp_path):
        import os, ishkarim_schema
        # create a minimal fake source dir
        src_dir = tmp_path / ishkarim_schema.MODULES[0]
...
class TestUtils:
    ...
    def test_read_work_md_missing(self, tmp_path):
        from ishkarim_schema.utils import read_work_md
    ...
    def test_extract_python_blocks(self):
        from ishkarim_schema.utils import extract_python_blocks
        md = """```python
def greet():
    return "hello world"
